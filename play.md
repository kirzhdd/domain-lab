...
### объяснение дипсика про деревья доменов, после которого стало понятнее
Представь, что ты владелец большой компании. У тебя есть несколько зданий, которые находятся на одной территории.

Домен — это одно здание. Внутри него — свои сотрудники (пользователи), свои компьютеры, свои правила безопасности (политики). Если нужно, ты можешь поставить отдельную охрану на входе в это здание. Это единица администрирования.

Дерево доменов — это несколько зданий, объединённых общим именем (как компания "Рога и Копыта"). Например, есть главный офис roga.ru, а его филиалы — msk.roga.ru, spb.roga.ru. Они разделяют общее пространство имён (имена заканчиваются на .roga.ru). Важно: у них автоматически возникают доверительные отношения. Если я могу зайти в главный офис, я могу зайти и в любой филиал. Это пространство имён.

Лес доменов — это территория компании, где могут стоять разные здания. Например, компания "Рога и Копыта" купила другой бренд — "Мягкие Лапы". У нового бренда свой домен softpaws.com, своё имя, свои сотрудники. Но теперь они находятся на общей территории (в одном лесу). Лес объединяет их общей схемой (одни и те же законы для всех), конфигурацией (общая карта всех зданий) и глобальным каталогом (единая база, где можно найти сотрудника из любого здания). Доверие между доменами внутри леса тоже есть, но оно устанавливается транзитивно (через корень). 
